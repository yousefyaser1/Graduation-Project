...
% ============================================================
\section{Variational Autoencoder (VAE) --- Anomaly Detection}
\label{sec:vae}
% ============================================================

Before any disease classification takes place, the system must first determine whether the captured image actually contains a skin abnormality. A Variational Autoencoder (VAE) serves as the first stage of the inference pipeline---an anomaly detection gate that filters out images of healthy skin before they reach the classifier.

The core insight behind this approach is straightforward: a VAE trained \textit{exclusively} on normal skin textures learns to reconstruct healthy skin with low error. When presented with a diseased patch, the reconstruction degrades noticeably, and the elevated error signals an anomaly.
...
\subsection{Sliding Window Anomaly Detection}

At inference time, the input image is divided into overlapping $64 \times 64$ patches using a stride of 32 pixels. Each patch is passed through the VAE, and the per-patch reconstruction error is computed as $\text{MSE}(\mathbf{x}, \hat{\mathbf{x}})$. If this error exceeds a calibrated threshold (0.008 in the TFLite deployment; see Section~\ref{sec:challenges}), the patch is flagged as anomalous.

The overall decision follows:

\begin{equation}
\text{Anomaly Ratio} = \frac{\text{Number of anomalous patches}}{\text{Total number of patches}}
\end{equation}

If the anomaly ratio exceeds 20\%, the image is classified as anomalous and forwarded to the CNN classifier. Otherwise, the pipeline short-circuits and returns a ``No skin disease detected'' result.

\subsection{VAE Testing and Validation}

To verify the anomaly detection module, two categories of test images were evaluated.

\subsubsection{Normal Skin Test Case}

\begin{itemize}
    \item Total patches analysed: 273
    \item Anomalous patches detected: 10
    \item Anomaly ratio: 0.037 (below the 0.20 threshold)
    \item \textbf{Result:} Correctly classified as \textbf{Normal}
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{Figures/Screenshot 2026-03-13 213036.png}
\caption{VAE reconstruction heatmap for a normal skin image. Uniformly low reconstruction error confirms normal skin texture.}
\label{fig:vae_normal}
\end{figure}

\subsubsection{Anomalous Skin Test Case}

\begin{itemize}
    \item Total patches analysed: 25
    \item Anomalous patches detected: 11
    \item Anomaly ratio: 0.44 (above the 0.20 threshold)
    \item \textbf{Result:} Correctly classified as \textbf{Anomalous}
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{Figures/Screenshot 2026-03-13 213558.png}
\caption{VAE reconstruction heatmap for an anomalous skin image. High-error regions (warm colours) correspond to visible lesion areas.}
\label{fig:vae_anomaly}
\end{figure}

These results confirm that the VAE effectively distinguishes between normal and diseased skin, serving as a reliable first-stage gate for the inference pipeline.
...
With the anomaly detection gate established, the next component is the disease classifier itself. This section documents the iterative model development process, beginning with MobileNetV2 as a lightweight baseline and progressing to EfficientNetB2 for improved accuracy.

\subsection{Training Configuration and Data Pipeline}
\label{sec:training_config}

All training pipelines shared a consistent data-loading pattern. Images were loaded using \texttt{tf.keras.utils.image\_dataset\_from\_directory} with categorical labels, then processed through an optimised pipeline (see Appendix~\ref{app:cnn_preprocessing}):
...
The pre-trained ImageNet backbone was imported without its original top layer (\texttt{include\_top=False}) and entirely frozen (\texttt{trainable=False}). A custom classification head was appended (see Appendix~\ref{app:cnn_head}):
...
Let the MobileNetV2 backbone contain $L = 155$ layers $\{l_1, l_2, \ldots, l_{155}\}$. The first 100 layers, which capture low-level features (edges, colour gradients), remained frozen. Layers 100--155, responsible for higher-level semantic features, were unfrozen and updated during training (see Appendix~\ref{app:finetune}):
...
EfficientNetB2 operates at a native resolution of $260 \times 260$ pixels and contains approximately 339 layers. Training followed the same two-phase protocol refined during the MobileNetV2 experiments (see Appendix~\ref{app:training_code}).
...
The trained EfficientNetB2 model achieved \textbf{90.46\% validation accuracy} on the 325-sample held-out set, with the following per-class breakdown:
...
\textbf{Overall} & \textbf{90.46\%} & \textbf{Deployed model} \\ \hline
\end{tabular}
\end{table}

This represents a gain of 6.3 percentage points over the best MobileNetV2 result, driven by EfficientNet's compound scaling and the switch to focal loss. The EfficientNetB2 model was selected as the production classifier and converted for on-device deployment.
...
All three models in the inference pipeline were converted from their training formats (Keras for EfficientNetB2, PyTorch for the VAE) to TensorFlow Lite for on-device execution. Conversion applied dynamic range quantisation via \texttt{tf.lite.Optimize.DEFAULT}, reducing model size and improving inference latency on mobile hardware.
...
\texttt{vae\_model.tflite} & VAE anomaly gate & $(1, 64, 64, 3)$ float32 & $[0, 1]$ \\ \hline
\texttt{cnn\_b2\_model.tflite} & EfficientNetB2 classifier & $(1, 260, 260, 3)$ float32 & $[0, 255]$ \\ \hline
\texttt{b2\_feature\_extractor.tflite} & Score-CAM features & $(1, 260, 260, 3)$ float32 & $[0, 255]$ \\ \hline
\end{tabular}
\end{table}

A critical implementation detail: EfficientNetB2 includes an internal \texttt{Rescaling(1/255)} layer baked into the model graph, so the application passes \textit{raw} $[0, 255]$ pixel values. The VAE, which was built without such a layer, requires pre-normalised $[0, 1]$ input. Mixing up these ranges was one of the most subtle bugs encountered during integration (see Section~\ref{sec:challenges} and Appendix~\ref{app:preprocessing}).
...
The three TFLite interpreters (VAE, B2 classifier, and B2 feature extractor) are loaded lazily on first call and held in memory for the application's lifetime. To maintain UI responsiveness during the 30-iteration Score-CAM loop, the computation yields to the Flutter event loop every five iterations:
...
%% SCREENSHOT: Open app → Welcome screen → capture showing logo, tagline, Get Started button.
...
%% SCREENSHOT: Login screen → email field, password field, login button.
...
%% SCREENSHOT: Home/Dashboard → main scan button, recent scans preview, navigation elements.
...
%% SCREENSHOT: Body Part Selection → anatomical figure (front view) with tappable regions.
...
%% SCREENSHOT: Capture screen → camera viewfinder, capture button, gallery icon.
...
%% SCREENSHOT: Analyzing screen → step indicators (Preprocessing, VAE, CNN, Score-CAM).
...
%% SCREENSHOT: Diagnosis Result → label, confidence %, heatmap overlay, probability bars.
...
%% SCREENSHOT: History screen → list of previous scans with diagnosis, date, confidence.
...
\textbf{Resolution:} Raw $[0, 255]$ pixel values are now passed to the EfficientNetB2 model, while $[0, 1]$ normalisation is retained only for the VAE. Two separate helper functions---\texttt{fillFloat32Raw()} and \texttt{fillFloat32()}---handle the two normalisation modes (see Appendix~\ref{app:preprocessing}).

\subsection{VAE Threshold Calibration Across Frameworks}

\textbf{Problem:} The VAE was originally calibrated in PyTorch with an MSE anomaly threshold of 0.006. After conversion to TFLite, a systematic offset of approximately +0.002 was observed, attributable to floating-point precision differences and dynamic range quantisation.

\textbf{Resolution:} The threshold was recalibrated empirically to 0.008 for the TFLite runtime, producing equivalent anomaly detection behaviour to the original PyTorch model.
...
Phase 1 & MobileNetV2 & Frozen backbone, GAP head & 75.16\% & No \\ \hline
Phase 2 & MobileNetV2 & Top 55 layers unfrozen & 84.16\% & No \\ \hline
Phase 3 & MobileNetV2 & Over-regularisation experiment & 73.91\% & No \\ \hline
Phase 4 & EfficientNetB0 & Focal loss, online augmentation & 84.62\% & No \\ \hline
\textbf{Final} & \textbf{EfficientNetB2} & \textbf{Focal loss, two-phase training} & \textbf{90.46\%} & \textbf{Yes} \\ \hline
\end{tabular}
\end{table}

The progression from MobileNetV2 (84.16\%) to EfficientNetB2 (90.46\%) represents a gain of 6.3 percentage points, driven primarily by compound scaling and the substitution of standard cross-entropy with focal loss for improved class-imbalance handling. The EfficientNetB2 model is the production-deployed classifier, converted to TFLite and bundled with the Flutter application as a fully offline, on-device inference engine.
