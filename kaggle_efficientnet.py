...
PHASE1_MODEL = "/kaggle/working/fifth_code_efficientnet_phase1.keras"
OUTPUT_MODEL = "/kaggle/working/fifth_code_efficientnet_model.keras"
...
# ---------------------------------------------------------------------------
# 3. Data pipeline (EfficientNetB0: no external preprocess_input)
#    Augmentation lives inside the model (Keras 3 compatible approach).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4. Build model (augmentation as first layers — only active during training)
# ---------------------------------------------------------------------------
def build_model(trainable_backbone=False, unfreeze_from=None):
    ...
    # Augmentation inside model: only runs when training=True (i.e. model.fit)
...
# ---------------------------------------------------------------------------
# 6. Phase 2
#    Load the saved Phase 1 checkpoint, then unfreeze in-place.
#    This preserves BatchNorm running statistics (avoids the accuracy drop
#    that happens when rebuilding the model from scratch and calling set_weights).
# ---------------------------------------------------------------------------
